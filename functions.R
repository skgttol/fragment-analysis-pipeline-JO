...
generate_custom_settings_file <- function(platemap_data, config, output_dir) {
  ...
  parsed_names <- stringr::str_split_fixed(settings_data$sample_name, "_", n = length(sample_cols))
  colnames(parsed_names) <- sample_cols
  parsed_names_df <- tibble::as_tibble(parsed_names)
  
  settings_data_parsed <- dplyr::bind_cols(settings_data, parsed_names_df)
  ...
  sym_time      <- rlang::sym(time_var)
  sym_primary   <- rlang::sym(primary_var)
  sym_secondary <- if(has_secondary) rlang::sym(secondary_var) else rlang::sym(primary_var)
  sym_batch     <- if(has_batch)     rlang::sym(batch_var)     else rlang::sym(primary_var)
  
  baseline_method <- config$parameters$baseline_grouping_level %||% "group"
  
  # 5. Create Group Column Logic
  tryCatch({
    
    # --- Custom Baseline Logic Helper ---
    # If method is "custom", we build the expression dynamically from the list
    custom_group_expr <- if(baseline_method == "custom") {
      ...
      # Check if vars exist
      missing <- setdiff(custom_vars, colnames(settings_data_parsed))
      if(length(missing) > 0) stop(paste("Missing custom baseline vars:", paste(missing, collapse=", ")))
      
      # Create interaction expression: paste(col1, col2, sep="-")
      rlang::expr(paste(!!!rlang::syms(custom_vars), sep = "_"))
    } else {
      NA # Placeholder
    }
    ...
    settings_prepared <- settings_data_parsed %>%
      dplyr::mutate(
        time_col = as.numeric(stringr::str_extract(as.character(!!sym_time), "\\d+\\.?\\d*")),
        
        group_col = dplyr::case_when(
          # A. Global Control
          baseline_method == "global_control" ~ config$parameters$global_control_group$level,
          
          # B. Genotype Only
          baseline_method == "genotype" ~ as.character(!!sym_primary),
          
          # C. CUSTOM (New Feature)
          baseline_method == "custom" ~ !!custom_group_expr,
          
          # D. Standard Grouping (Primary + Secondary + Batch)
          has_secondary && has_batch ~ paste(!!sym_primary, !!sym_secondary, !!sym_batch, sep = "_"),
          has_secondary && !has_batch ~ paste(!!sym_primary, !!sym_secondary, sep = "_"),
          !has_secondary && has_batch ~ paste(!!sym_primary, !!sym_batch, sep = "_"),
          
          # E. Fallback
          TRUE ~ as.character(!!sym_primary)
        )
      ) 
    ...
    final_settings_file <- settings_prepared %>%
      dplyr::transmute(
        ...
        flank = cs$flank,
        repeat_unit = cs$repeat_unit,
        correction = cs$correction,
        pathogenic_threshold = cs$pathogenic_threshold,
        ii_threshold_method = cs$ii_threshold_method,
        ii_threshold_height = cs$ii_threshold_height,
        ii_threshold_area = cs$ii_threshold_area,
        target_channel = cs$target_channel,
        target_start = cs$target_start,
        target_end = cs$target_end,
        plot_start = cs$plot_start,
        plot_end = cs$plot_end,
        control_rpt = cs$control_rpt %||% "",
        group = group_col,
        group_control_sample = cs$group_control_sample_prefix %||% "",
        time = time_col,
        exclude = ""
      )
    ...
    for (col in names(defaults)) {
      if (!col %in% colnames(final_settings_file)) {
        val <- if(!is.null(cs[[col]])) cs[[col]] else defaults[[col]]
        final_settings_file[[col]] <- val
      }
    }
    
    if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
    file_name <- paste0(format(Sys.Date(), "%Y-%m-%d"), "_", format(Sys.time(), "%H%M"), "_", "romeo_settings.xlsx")
...
run_statistical_model <- function(data_summary, config, response_variable) {
  ...
  try({
    ...
    model_results[["Group_Expansion_Rates"]] <- format_table(emm_df)
    
    emm_pairs <- as.data.frame(graphics::pairs(em_trends)) 
    model_results[["Rate_Comparisons"]] <- format_table(emm_pairs) %>% dplyr::rename(Rate_Difference = Estimate)
...
build_label_columns <- function(data, baselines, config, extra_dfs = list()) {
  ...
  # ── Step 1: Apply value_renaming to primary column BEFORE building labels ──
  rename_map <- NULL
  ...
  # Apply renaming to primary column in all data frames first
  apply_rename_primary <- function(df) {
    if (!is.null(df) && primary_var %in% names(df)) {
      df[[primary_var]] <- rename_col(df[[primary_var]])
    }
    df
  }
  
  data     <- apply_rename_primary(data)
  ...
  # ── Step 3: Determine display order ──
  manual_levels_raw <- config$factor_levels[[primary_var]]
  
  if (!is.null(manual_levels_raw)) {
    # Translate raw config levels through rename_map so they match the
    # already-renamed values in the data.
    manual_levels_renamed <- if (!is.null(rename_map)) {
      dplyr::coalesce(unname(rename_map[as.character(manual_levels_raw)]),
                      as.character(manual_levels_raw))
    } else {
      as.character(manual_levels_raw)
    }
    logr::log_print(paste(
      "build_label_columns(): enforcing config factor order for", primary_var,
      "->", paste(manual_levels_renamed, collapse = ", ")
    ))
    genotype_map <- genotype_map %>%
      dplyr::filter(as.character(!!sym_primary) %in% manual_levels_renamed) %>%
      dplyr::arrange(match(as.character(!!sym_primary), manual_levels_renamed))
  } else {
    logr::log_print(paste(
      "build_label_columns(): no manual levels for", primary_var,
      "- auto-sorting by average CAG baseline."
    ))
    genotype_map <- genotype_map %>% dplyr::arrange(avg_baseline)
  }
  
  # ── Step 4: Build label strings using config templates ──
  fmt_pub <- config$label_formats$publication %||% "{geno}"
  fmt_exp <- config$label_formats$exploratory %||% "{geno} (Start: {base}Q)"
  
  genotype_map <- genotype_map %>%
    dplyr::mutate(
      Label_Pub = as.character(glue::glue(fmt_pub,
                                          base = avg_baseline, geno = !!sym_primary)),
      Label_Exp = as.character(glue::glue(fmt_exp,
                                          base = avg_baseline, geno = !!sym_primary))
    )
  
  logr::log_print("build_label_columns(): genotype map (in display order):")
  logr::log_print(as.data.frame(genotype_map))
  
  # ── Step 5: Attach label columns and set factor levels ──
  ordered_primary <- as.character(genotype_map[[primary_var]])
  ordered_pub     <- genotype_map$Label_Pub
  ordered_exp     <- genotype_map$Label_Exp
  
  attach_labels <- function(df) {
    if (is.null(df) || !primary_var %in% names(df)) return(df)
    df %>%
      # Remove stale label columns so left_join is clean
      dplyr::select(-dplyr::any_of(c("Genotype_Pub", "Genotype_Exp",
                                     "avg_baseline", "Label_Pub", "Label_Exp"))) %>%
      dplyr::left_join(
        genotype_map %>%
          dplyr::select(!!sym_primary, avg_baseline, Label_Pub, Label_Exp),
        by = primary_var
      ) %>%
      dplyr::mutate(
        # Re-level primary column to the display order
        !!sym_primary  := factor(as.character(!!sym_primary),  levels = ordered_primary),
        # Build ordered label factors
